...
async def save_emergency_contact(user_id: int, first_name: str, phone_number: str, city: str) -> bool:
    """حفظ بيانات الطوارئ للعضو (تطوعي وسري)"""
    if not supabase:
        return False
    try:
        data = {
        ...
        await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: supabase.table("emergency_contacts").upsert(data).execute()
        )
        return True
    except Exception as e:
        print(f"خطأ في حفظ بيانات الطوارئ: {e}")
        return False

async def get_emergency_contact(user_id: int) -> dict:
    """استرجاع بيانات طوارئ لعضو محدد (للمشرفين فقط)"""
    if not supabase:
        return None
    try:
        result = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: supabase.table("emergency_contacts")
                .select("*")
        ...
        print(f"خطأ في جلب بيانات الطوارئ: {e}")
        return None

async def delete_emergency_contact(user_id: int) -> bool:
    """حذف بيانات الطوارئ (للعضو نفسه)"""
    if not supabase:
        return False
    try:
        await asyncio.get_event_loop().run_in_executor(
        ...
        print(f"خطأ في حذف بيانات الطوارئ: {e}")
        return False

async def is_admin_user(user_id: int) -> bool:
    """التحقق مما إذا كان المستخدم مشرف طوارئ (مخول بالاطلاع على البيانات)"""
    if not supabase:
        return False
    try:
        result = await asyncio.get_event_loop().run_in_executor(
